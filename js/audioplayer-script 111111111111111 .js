...
elGain.onmousedown = function(e) { 
	...
	function moveAt(e) { 
		...
		$('.gain-red-timeline').css({'width':parseFloat(elGain.style.left) + 1 + '%'}) 
	} 
...
////////////////////////////////////////////////////////////////
	...
	let cover = {
			a0: document.getElementById('a0').style.backgroundImage = "url('img/audioplayer/covers/с1.jpg')",
			a1: document.getElementById('a1').style.backgroundImage = "url('img/audioplayer/covers/с2.jpg')",
			a2: document.getElementById('a2').style.backgroundImage = "url('img/audioplayer/covers/с1.jpg')",
			a3: document.getElementById('a3').style.backgroundImage = "url('img/audioplayer/covers/с2.jpg')",
			a4: document.getElementById('a4').style.backgroundImage = "url('img/audioplayer/covers/с1.jpg')",
			a5: document.getElementById('a5').style.backgroundImage = "url('img/audioplayer/covers/с2.jpg')",
			a6: document.getElementById('a6').style.backgroundImage = "url('img/audioplayer/covers/с1.jpg')",
			a7: document.getElementById('a7').style.backgroundImage = "url('img/audioplayer/covers/с2.jpg')",
			a8: document.getElementById('a8').style.backgroundImage = "url('img/audioplayer/covers/с1.jpg')",
			a9: document.getElementById('a9').style.backgroundImage = "url('img/audioplayer/covers/с2.jpg')",
			a10: document.getElementById('a10').style.backgroundImage = "url('img/audioplayer/covers/с1.jpg')",
			a11: document.getElementById('a11').style.backgroundImage = "url('img/audioplayer/covers/с3.jpg')",
			
		}

	var id_song, Song, vol=1 //vol для сейва громкости при переключении песни
	var tracklist = [
	...
	let lastRandoms = []
		...
		//		console.log((parseInt(Song.currentTime/60)+':'+parseInt(Song.currentTime%60)))
				cur=100-((parseInt(Song.duration)-Math.ceil(Song.currentTime))*100)/parseInt(Song.duration)
				/*console.log(cur)* текущее время на таймлайне */
				$('.red-timeline').css({'width':cur+'%'})
				if(curtime%60 < 10) {
					$('.first-time').text((parseInt(curtime/60)+":0"+parseInt(curtime%60)))
				}
				else {
					$('.first-time').text((parseInt(curtime/60)+":"+parseInt(curtime%60)))
				}
				...
				if(Song.currentTime == this.duration) {
					document.getElementById('stopSong').style.backgroundImage = "url('img/audioplayer/play.svg')"
	    			Song.pause()
				}
			});
			...
			Song.addEventListener('loadedmetadata', function() {
				...
				if(this.duration%60 < 10) {
					$('.second-time').text((parseInt(this.duration/60)+":0"+parseInt(this.duration%60 + 1)))
				}
				else {
					$('.second-time').text((parseInt(this.duration/60)+":"+parseInt(this.duration%60 + 1)))
				}
			});
			}
		...
		function swapPicture(k) {
			for (i=0; i < keysFromCover.length; i++) { //замена картинки на играющий сейчас трек
...
document.getElementById('allTracks').onclick = function() {
    		...
    		function open(e) {
	    		...
	    		for (i=0; i < tracklist.length; i++) { 
	    			if (tracklist[i][0] == e.target.id) {  //включает трек
	    				if (tracklist[i][0] !== lastPlay) { //чтобы при игре трека и повторном нажатии на картинку трек не начинался заново
	    					id = tracklist[i][0]
		    				if (Song) {
			    				if (id == lastPlay) {
	...
	if(Song) {
		...
		$('.gain-timeline').on('mousemove', function (e) {
			...
			$('.gain-timeline').on('click', function () {
				...
				elGain.style.left = parseFloat(xproc*88/100)+ "px"
		...
		$('.gain-red-timeline').on('mousemove', function (e) {
			...
			$('.gain-red-timeline').on('click', function () {
				...
				console.log("1")
					...
					elGain.style.left = parseFloat(x*88/91)+ "px"
					x_elGain = elGain.style.left 
...
$('.stop').on('click', function () { //пауза песни
	   console.log(x)
	    if (Song.paused) {
	    	document.getElementById('stopSong').style.backgroundImage = "url('img/audioplayer/stop.svg')"
	   	 	Song.play()
	    }
	    else {
	    	document.getElementById('stopSong').style.backgroundImage = "url('img/audioplayer/play.svg')"
...
$('.left').on('click', function () { //левее
	  ...
	  if (x == 0) {
	  	...
	  	document.getElementById('stopSong').style.backgroundImage = "url('img/audioplayer/stop.svg')" //чтобы в случае нажатия кнопка play не оставалась
...
$('.right').on('click', function () { //правее
	  let newId = id.slice(1)
	  if (x == 11) {
	  	Song.pause(id)
	  	newId = "a"+0
...
$('.random-button').on('click', function () { //рандом трека
	if(Song){
		Song.pause(id)
	}
	...
	if(lastRandoms.length > 0 && lastRandoms.length < 8) {
				...
				for (i=0; i < lastRandoms.length; i++) {    //попытка разнообразить рандом
					if(x == lastRandoms[i]) {
